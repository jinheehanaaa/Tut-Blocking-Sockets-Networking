...
namespace PNet {
	...
	IPEndpoint::IPEndpoint(const char* ip, unsigned short port)
	{
		...
		if (result == 1)
		{
			// Erorr handling for user connected to ip address instead of hostname
			if (addr.S_un.S_addr != INADDR_NONE)
			{
				ip_string = ip;
				hostname = ip; // We do not have our domain yet
	...
	IPEndpoint::IPEndpoint(sockaddr* addr)
	{
		...
		ipversion = IPVersion::IPv4;
		port = ntohs(addrv4->sin_port); // Convert network to host byte order
		ip_bytes.resize(sizeof(ULONG));
		memcpy(&ip_bytes[0], &addrv4->sin_addr, sizeof(ULONG));

		// Convert ip into presentation format & store into ip_string
		ip_string.resize(16);
